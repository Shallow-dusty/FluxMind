...
python scripts/product_registry.py status --format markdown # 本地 user/workspace/quota/billing ledger 与 quota guard 状态
...
- `src/product_registry.py` — 可选本地 SQLite product registry；提供 user/workspace/quota/usage/billing attribution ledger，并在显式启用 `FLUXMIND_PRODUCT_QUOTA_GUARD_ENABLED=true` 时供 `/query*` 路径做本地 quota guard；不连接外部身份或支付系统。
...
- `src/product_readiness.py` + `src/provider_readiness.py` — no-secret activation readiness：前者覆盖身份/API-key/配额/计费并能检查本地 SQLite key registry、product registry 与本地 quota guard，后者覆盖外部图像 provider、托管执行、MATLAB backend 和 provider quota guard。默认只报告 blocker code，不启用外部调用。
