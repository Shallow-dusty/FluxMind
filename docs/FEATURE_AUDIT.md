...
.venv/bin/python scripts/health_check.py                             pass, local/docs/query-latency/query-alert/
                                                                      ...
                                                                      job-owner-metadata-projection/
                                                                      job-request-id-projection/
...
The 2026-06-20 23:12 CST job owner metadata projection audit adds focused
coverage for public `JobResponse` owner metadata. Exact job responses now
replace raw submitted `owner_id` and `owner_label` values with presence, short
fingerprint, and `owner_exported=false` fields for create, exact fetch, retry,
image-generation, async job, and corpus-profile rebuild responses. Internal
durable job records still retain normalized owner metadata for local filters,
retry inheritance, artifact attribution, and product guard inputs. Tests verify
that secret-like owner IDs and labels are not echoed while retry preserves
owner fingerprints; local verification passes with 630 tests, 89% branch
coverage, offline RAG eval, OpenAPI no-secret snapshot `diff_count=0`, and
storage-schema 0 problems.

The 2026-06-20 23:21 CST job request ID projection audit adds focused coverage
for public `JobResponse` request ID metadata. Exact job responses and job-list
summaries now pass persisted request IDs through the same runtime request-ID
sanitizer used by runtime-event projections. Safe current correlation IDs remain
available, while unsafe legacy values such as bearer/token-like headers are
reduced to `request_id_present=true` and `request_id_redacted=true` without a
raw `request_id` field. Tests verify that a legacy `Bearer secret-request-token`
...
The 2026-06-21 00:03 CST share-link create error-detail audit adds the same
fixed-code public error guard to `POST /admin/share-links`. Registry
`ValueError` failures no longer populate `detail.code` with `str(exc)`; the
route now returns `public_error_detail("invalid_share_link_request")`. Regression
coverage injects an exception containing a local path, secret-like token, and
private resource reference and confirms the 400 response does not echo any of
...
Local job and worker bridge   verified      immediate and async job routes, idempotency keys,
                                            ...
                                            observability.
                                            Raw owner IDs and labels are also
                                            not echoed in exact responses;
                                            presence, fingerprint, and
                                            owner_exported=false fields
                                            preserve local attribution
                                            observability.
                                            Unsafe legacy request IDs are
                                            reduced to request_id_present and
                                            request_id_redacted flags, while
                                            safe current correlation IDs remain
                                            available.
