...
The explicit Streamlit product-registry management panel now uses the same
no-secret error projection for OSError, SQLite, and validation failures, so UI
...
body values from public 422 responses.
...
A 2026-06-20 21:50 CST API validation error audit refresh confirms the updated
local checkout with 622 tests passing, 89% branch coverage, offline RAG eval
passing, OpenAPI no-secret snapshot drift still at `diff_count=0`, and storage
schema drift still at 0 problems.
A 2026-06-20 22:01 CST git/documentation drift refresh confirms the clean
checkout after the API validation docs sync, with 622 tests passing, 89% branch
...
A 2026-06-20 22:21 CST API request validation projection audit confirms the
updated local checkout with 626 tests passing, 89% branch coverage, offline RAG
...
projection hardening pass; no production deployment or live Trace-Twin service
facts were refreshed.
A 2026-06-21 00:14 CST deploy sync CLI error-output audit confirms the updated
local checkout with 635 tests passing, 89% branch coverage, local health/docs
anchors passing, and `git diff --check` clean. `scripts/deploy_sync.py` now
...
enabling the SQLite registry alone does not expose public UI write controls, and
they sanitize OSError, SQLite, and validation error output through the no-secret
error boundary before rendering operator messages.
