...
reconfirmed those boundaries, and the Streamlit job-result panel now sanitizes
failed job messages before display. The Streamlit corpus profile report panel
...
The guarded deploy-sync CLI also sanitizes preflight error output.
...
Gate                                      Result
----------------------------------------  -------------------------------------
Deploy sync error-output audit            2026-06-21 00:14 CST, checkout at
                                          e44f615 before this docs refresh;
                                          no production deployment performed
Share-link create error audit             2026-06-21 00:03 CST, checkout at
                                          4a43f14 before this docs refresh;
                                          no production deployment performed
Streamlit profile report error audit      2026-06-20 23:53 CST, checkout at
                                          0bbf9ff before this docs refresh;
                                          no production deployment performed
Streamlit job result error audit          2026-06-20 23:42 CST, checkout at
                                          125664d before this docs refresh;
                                          no production deployment performed
...
Job idempotency key projection audit      2026-06-20 23:01 CST, checkout at
                                          ...
                                          no production deployment performed
Job detail API projection audit           2026-06-20 22:43 CST, checkout at
                                          ddd17b1 before this docs refresh;
                                          no production deployment performed
Streamlit validation error-output audit   2026-06-20 22:30 CST, checkout at
                                          673cd2f before this docs refresh;
                                          no production deployment performed
API request validation projection audit   2026-06-20 22:21 CST, checkout at
                                          4bf9775 before this docs refresh;
                                          no production deployment performed
Index rebuild job projection audit        2026-06-20 22:10 CST, checkout at
                                          19de06f before this docs refresh;
                                          no production deployment performed
...
exported=false flags rather than public raw values. Persisted request IDs are
also projected through the runtime request-ID sanitizer, so unsafe legacy values
become presence/redaction flags while safe current correlation IDs remain
available.
...
when the SQLite backend is explicitly enabled. The Streamlit product-registry
operator panel also requires its own explicit management flag and sanitizes
OSError, SQLite, and validation error output through the no-secret error
boundary instead of rendering raw exception strings.
