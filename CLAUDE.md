...
- `src/product_registry.py` — 可选本地 SQLite product registry；提供 user/workspace/RBAC/quota/usage/billing attribution ledger，在显式启用 `FLUXMIND_PRODUCT_QUOTA_GUARD_ENABLED=true` 时供 `/query*` 路径做本地 quota guard，在显式启用 `FLUXMIND_PRODUCT_RBAC_GUARD_ENABLED=true` 时按 workspace role 守护查询、job/corpus/admin 写路径；不连接外部身份或支付系统。
