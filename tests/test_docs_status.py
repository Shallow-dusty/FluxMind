...
def test_repo_status_records_post_deployment_git_boundary():
    ...
    assert "Deployment record follow-up    32fca21 after live verification and docs-only sync" in text
...
def test_roadmap_near_term_plan_starts_from_deployed_baseline():
    ...
    assert "Treat `32fca21` as the current pushed/deployed no-key baseline" in text
