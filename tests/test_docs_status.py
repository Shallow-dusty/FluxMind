...
def test_repo_status_records_post_deployment_git_boundary():
    ...
    assert "Deployed source baseline       3cfa426 fix: reduce FluxMind API import latency" in text
    assert "Deployment record follow-up    this docs refresh after 2026-06-15 03:15 live verification" in text
...
def test_roadmap_near_term_plan_starts_from_deployed_baseline():
    ...
    assert "Treat `3cfa426` as the current pushed/deployed no-key baseline" in text
