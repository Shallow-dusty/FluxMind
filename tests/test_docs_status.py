...
def test_repo_status_records_post_deployment_git_boundary():
    ...
    assert "Current implementation commit  51fee7e fix: harden local artifact path resolution" in text
    assert "Current docs/health sync       f2d2da1 docs: record corpus metadata audit status" in text
    ...
    assert "Current local app-code HEAD    51fee7e fix: harden local artifact path resolution" in text
