...
def test_repo_status_records_post_deployment_git_boundary():
    ...
    assert "Verified repo baseline         3b8ecd7 docs: record runtime hardening deployment" in text
    assert "Last deployed app baseline     4f27651 fix: exclude coverage data from deploy sync" in text
    assert "Live verification follow-up    read-only checks refreshed on 2026-06-15 08:32 CST" in text
