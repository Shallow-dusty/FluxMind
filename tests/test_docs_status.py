...
def test_repo_status_records_post_deployment_git_boundary():
    ...
    assert "Verified repo baseline         1b7795a test: calibrate live retrieval refs" in text
    assert "Last deployed source/eval baseline 1b7795a test: calibrate live retrieval refs" in text
    assert "Live verification follow-up    14-paper corpus rebuild and 74/74 live retrieval refreshed on 2026-06-15 10:23 CST" in text
...
def test_roadmap_near_term_plan_starts_from_deployed_baseline():
    ...
    assert "Treat `1b7795a` as the current pushed/deployed source/eval baseline" in text
