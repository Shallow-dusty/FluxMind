...
def test_repo_status_records_post_deployment_git_boundary():
    ...
    assert "Current implementation commit  ddd17b1 fix: redact job detail code outputs" in text
    assert "Current docs/health sync       docs: record job detail projection audit status (this commit)" in text
    assert "by the forty-four local commits below" in text
    ...
    assert "Current local app-code HEAD    ddd17b1 fix: redact job detail code outputs" in text
    assert "docs: record job detail projection audit status (this commit)" in text
...
def test_roadmap_near_term_plan_starts_from_deployed_baseline():
    ...
    assert "pass, 628 tests, 2 known warnings" in text
