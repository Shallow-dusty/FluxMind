...
def test_repo_status_records_post_deployment_git_boundary():
    ...
    assert "Current implementation commit  98daedc fix: sanitize Streamlit query errors" in text
    assert "Current docs/health sync       docs: record Streamlit query error audit status (this commit)" in text
    assert "by the sixty-three local commits below" in text
    ...
    assert "Current local app-code HEAD    98daedc fix: sanitize Streamlit query errors" in text
    assert "docs: record Streamlit query error audit status (this commit)" in text
    assert "98daedc fix: sanitize Streamlit query errors" in text
    assert "bfca2e6 docs: record readiness cli data error audit status" in text
    assert "41ca43f fix: sanitize readiness cli data errors" in text
    assert "fa9d482 docs: record deploy sync error audit status" in text
    ...
    assert "2458fd8 docs: record job detail projection audit status" in text
    assert "de19eda test: isolate code execution event assertion" in text
    assert "ddd17b1 fix: redact job detail code outputs" in text
    assert "51230f8 docs: record Streamlit validation error audit status" in text
    ...
    assert "Streamlit query error-output follow-up on 2026-06-21 00:37 CST" in text
    assert "main...origin/main [ahead 62]" in text
    assert "pass, 641 tests" in text
    assert "safe_streamlit_status_message(error.message, fallback=error.code)" in text
    assert "rerun serially because" in text
    assert "Readiness CLI data-error output follow-up on 2026-06-21 00:27 CST" in text
    assert "main...origin/main [ahead 60]" in text
    assert "pass, 640 tests" in text
    assert "format_cli_error(exc)" in text
    assert "JSON/ValueError data errors" in text
    ...
    assert 'detail={"code": str(exc)}' in text
    assert "invalid_share_link_request" in text
    assert "secret-like token" in text
    ...
    assert "sk-secret-owner-id-12345678" in text
    assert "raw owner_id/owner_label exported=false" in text
    assert "Job idempotency key projection follow-up on 2026-06-20 23:01 CST" in text
    assert "main...origin/main [ahead 45]" in text
    assert "pass, 629 tests" in text
    assert "sk-secret-idempotency-key-verify-12345678" in text
    assert "raw idempotency_key exported=false" in text
    assert "Job detail API projection follow-up on 2026-06-20 22:43 CST" in text
    assert "main...origin/main [ahead 43]" in text
    assert "pass, 628 tests" in text
    assert "sk-secret-job-output-verify" in text
    assert "secret-main.py, stdout, stderr" in text
...
def test_roadmap_near_term_plan_starts_from_deployed_baseline():
    ...
    assert "pass, 641 tests, 2 known warnings" in text
    assert "Streamlit query error-output audit" in text
    assert "Streamlit-query-error-sanitizer" in text
    assert "Readiness CLI data-error audit" in text
    assert "readiness-CLI-data-error-sanitizer" in text
    ...
    assert "job-idempotency-key-projection" in text
    assert "Job detail API projection audit" in text
    assert "job-detail-code-output-projection" in text
