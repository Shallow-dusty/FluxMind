#!/usr/bin/env python3
"""Run a no-secret local product activation rehearsal."""
...
from src.product_activation_rehearsal import (  # noqa: E402
...
def main() -> int:
    ...
    parser.add_argument(
        "--require-activation",
        action="store_true",
        help="Exit nonzero unless the local activation rehearsal passes.",
    )
    args = parser.parse_args()

    try:
        status = collect_product_activation_rehearsal(root=args.root)
        if args.format == "markdown":
            output = format_product_activation_rehearsal_markdown(status) + "\n"
        else:
    ...
    if args.require_activation and not status.get("ok"):
        return 1
